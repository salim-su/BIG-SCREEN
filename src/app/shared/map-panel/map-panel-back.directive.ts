...
@Directive({
  selector: '[appMapPanel1]'
})
export class MapPanelBackDirective {
  ...
  loadRecordBulk() {
    this.httpClient.get('tjpn4-bulk/tallyStack/list').subscribe(mapInfo => {
  ...
  loadArea(pointArr: any): any {
    const vectorSourceOld = new VectorSource();
    const polygon = new olGeom.Polygon(pointArr);
